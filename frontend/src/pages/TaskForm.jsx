...
export default function TaskForm({ mode }) {
  ...
  const [form, setForm] = useState({
    title: "", description: "", deadline: "", effort: 5, complexity: 5,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.effort < 1 || form.effort > 10) { setError("Effort must be between 1 and 10."); return; }
    if (form.complexity < 1 || form.complexity > 10) { setError("Complexity must be between 1 and 10."); return; }
    setLoading(true);
    ...
    try {
      if (isEdit) { await client.put(`tasks/${id}/`, payload); }
        ...
        <div className="card task-form-card">
          ...
          <p className="form-page-subtitle">
            {isEdit
              ...
              : "Fill in the details and we'll calculate the priority score for you."}
          </p>
          ...
          <form onSubmit={handleSubmit}>
                ...
                <div className="score-display" style={{ marginTop: "0.6rem" }}>
                  <div className="score-dots">
                    {effortDots.map((active, i) => (
                      <span key={i} className={`score-dot ${active ? "active-effort" : ""}`} />
                    ))}
                  </div>
                </div>
              </div>
                ...
                <div className="score-display" style={{ marginTop: "0.6rem" }}>
                  <div className="score-dots">
                    {complexityDots.map((active, i) => (
                      <span key={i} className={`score-dot ${active ? "active-complexity" : ""}`} />
                    ))}
                  </div>
                </div>
              </div>
            ...
            <div style={{
              ...
              <code style={{ opacity: 0.85 }}>(1 / (days_left + 1)) × 100 + effort × 2 + complexity</code>
              <br />
